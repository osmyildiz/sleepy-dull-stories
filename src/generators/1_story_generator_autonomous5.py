...
class ToibinStoryGenerator:
    ...
    def enforce_character_limit(self, story_content: str, scene_info: Dict, max_chars: int = 3000) -> str:
        """Enforce strict 3000 character limit for ElevenLabs compatibility"""

        current_length = len(story_content)

        # If under 2800, try to expand
        if current_length < 2800:
            print(f"⚠️ Scene {scene_info.get('scene_id')} too short ({current_length} chars) - expanding...")
            expanded = self._expand_story(story_content, scene_info, 2800)
            return expanded

        # If over 3000, truncate
        elif current_length > max_chars:
            print(f"⚠️ Scene {scene_info.get('scene_id')} too long ({current_length} chars) - truncating...")
            truncated = self._smart_truncate(story_content, max_chars)
            print(f"✅ Truncated to {len(truncated)} chars")
            return truncated

        # If between 2800-3000, perfect
        else:
            print(f"✅ Scene {scene_info.get('scene_id')} perfect length ({current_length} chars)")
            return story_content

    def _smart_truncate(self, content: str, max_chars: int) -> str:
        """Smart truncation that preserves sound effects and story flow"""

        # Target 2950 chars to leave buffer
        target_chars = min(max_chars - 50, 2950)

        if len(content) <= target_chars:
            return content

        print(f"🔧 Smart truncating from {len(content)} to ~{target_chars} chars")

        # Split by sentences (better sentence detection)
        ...
        for i, sentence in enumerate(sentences):
            ...
            test_length = len(result) + len(sentence)

            if test_length > target_chars:
                print(f"💡 Stopping at sentence {i + 1}/{len(sentences)}")

                # Add a natural closing with sound effect if there's room
                closing_options = [
                    " [final breath] And peace settled over the scene.",
                    " [gentle silence] The moment faded into tranquil memory.",
                    " [soft exhale] Time seemed to pause in quiet contemplation.",
                    " [peaceful fade] The story continued in whispered dreams."
                ]

                # Find shortest closing that fits
                for closing in closing_options:
                    if len(result) + len(closing) <= target_chars:
                        result += closing
                        print(f"✅ Added natural closing: {closing[:30]}...")
                        break
        ...
        if result and not result.endswith(('.', '!', '?', ']')):
            if len(result) < target_chars - 1:
                result += "."

        print(f"✅ Final truncated length: {len(result)} chars")
        return result.strip()

    def _expand_story(self, content: str, scene_info: Dict, min_chars: int) -> str:
        """Expand short story with more atmospheric details and sound effects"""

        if len(content) >= min_chars:
            return content

        # Add more atmospheric content before the ending
        expansion_parts = [
            f"[gentle wind] The atmosphere grew more contemplative as moments passed.",
            f"[soft footsteps] Each step seemed to echo with the weight of history.",
            f"[distant sounds] The world beyond felt both near and infinitely far away.",
            f"[thoughtful pause] In this quiet space, time itself seemed to slow."
        ]

        # Find the last sentence and insert expansion before it
        sentences = content.split('.')
        if len(sentences) >= 2:
            main_content = '.'.join(sentences[:-1]) + '.'
            last_sentence = sentences[-1]

            expansion_needed = min_chars - len(content)
            expansion_text = ""

            for part in expansion_parts:
                if len(expansion_text) + len(part) < expansion_needed:
                    expansion_text += " " + part
                else:
                    break

            expanded = main_content + expansion_text + " " + last_sentence
            return expanded
    ...
    **HISTORICAL MOMENT STRUCTURE:**
    ...
    - MAXIMUM 3000 characters per story (including sound effects)
    - TARGET RANGE: 2800-2950 characters per story
    - Stories under 2800 chars will be expanded automatically
    - Stories over 3000 chars will be truncated automatically
    ...
    {{
      ...
      "chunk_stats": {{
        ...
        try:
            ...
            # ENFORCE CHARACTER LIMIT (2800-3000 chars)
            if 'stories' in parsed_result:
                for story_id, story_content in parsed_result['stories'].items():
                    # Find scene info for this story
                    scene_info = next((s for s in chunk_scenes if str(s.get('scene_id')) == story_id), {})

                    # Apply character limit enforcement
                    parsed_result['stories'][story_id] = self.enforce_character_limit(
                        story_content, scene_info, 3000
                    )
    ...
    {{
      ...
      "chunk_stats": {{
        ...
        try:
            ...
            # ENFORCE CHARACTER LIMIT (2700-3000 chars)
            if 'stories' in parsed_result:
                for story_id, story_content in parsed_result['stories'].items():
                    # Simple scene info for Stage 3
                    scene_info = {'scene_id': story_id, 'title': f'Scene {story_id}'}
...
if __name__ == "__main__":
    try:
        ...
        if test_mode:
            print("\n🧪 TESTING REAL STORY GENERATION WITH CHARACTER LIMIT")
            generator = ToibinStoryGenerator()

            # Test scene for real generation (WITH ALL REQUIRED FIELDS)
            test_scene = {
                'scene_id': 1,
                ...
                'internal_focus': 'Accepting her fate with dignity',
                'tóibín_element': 'Quiet resignation and understated emotion',
                'scene_description': 'Cleopatra in her final moments of contemplation',
                'environmental_details': 'Silk curtains, marble floors, golden morning light',
                'emotional_core': 'Peaceful acceptance of destiny'
            }

            print(f"🎬 Generating REAL story with CHARACTER LIMIT control...")
            print(f"   Scene: {test_scene['title']}")
            print(f"   Character: {test_scene['main_character']}")
            print(f"   Target: 2800-3000 characters with sound effects")

            # Create a mini chunk with just this scene
            chunk_scenes = [test_scene]
            result = {}  # Initialize result

            try:
                # Call the real chunk generation function
                result = generator._generate_story_chunk(
                    "Cleopatra's Final Night",
                    "The last hours of Egypt's final pharaoh",
                    chunk_scenes,
                    0,  # chunk_index
                    None,  # hook_content
                    None,  # subscribe_content
                    1,  # total_scenes
                    1  # stage_total
                )
                ...
                if 'stories' in result and result['stories']:
                    for story_id, story_content in result['stories'].items():
                        char_count = len(story_content)
                        word_count = len(story_content.split())

                        print(f"📊 SCENE {story_id} STATS:")
                        print(f"   Characters: {char_count}")
                        print(f"   Words: {word_count}")
                        print(f"   Estimated Duration: {word_count / 140:.1f} minutes")

                        # Check character limit
                        if char_count < 2800:
                            print(f"   ⚠️ TOO SHORT - needs expansion")
                        elif char_count > 3000:
                            print(f"   ⚠️ TOO LONG - needs truncation")
                        else:
                            print(f"   ✅ PERFECT LENGTH")

                        print(f"\n📖 STORY CONTENT:")
                        print("-" * 50)
                        if len(story_content) > 800:
                            print(story_content[:800] + "...")
                        else:
                            print(story_content)
                        print("-" * 50)

                        # Check for sound effects
                        sound_effects = story_content.count('[')
                        print(f"\n🎭 SOUND DESIGN CHECK:")
                        print(f"   Sound Effects Found: {sound_effects} opening brackets")

                        # Show first few sound effects
                        import re
                        effects = re.findall(r'\[([^\]]+)\]', story_content)
                        if effects:
                            print(f"   Sample Effects: {effects[:5]}")
                        else:
                            print(f"   ❌ NO SOUND EFFECTS FOUND!")
                ...
                import traceback
                traceback.print_exc()
                result = {}  # Set empty result for error case

            print(f"\n🎯 CHARACTER LIMIT SYSTEM TEST:")
            print(f"✅ Real story generation: {'WORKING' if result and 'stories' in result else 'FAILED'}")
            print(f"✅ Character limit control: {'CHECK ABOVE' if result and 'stories' in result else 'NOT TESTED'}")
            print(f"✅ Sound effects integration: {'CHECK ABOVE' if result and 'stories' in result else 'NOT TESTED'}")

            continue_full = input(f"\n🚀 Continue to full production? (y/n): ").lower() == 'y'
