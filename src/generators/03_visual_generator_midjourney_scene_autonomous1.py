...
Please provide the corrected prompt that will pass Midjourney's content policy."""
    ...
    def update_visual_prompts_json(self, visual_prompts_path: str, scene_number: int, corrected_prompt: str) -> bool:
        ...
        try:
            ...
            # BURADA DÜZELTİLMELİ:
            # JSON'daki yapıyı kontrol et
            if "visual_prompts" in prompts_data:
                scenes_list = prompts_data["visual_prompts"]
            else:
                scenes_list = prompts_data.get("scenes", [])

            # Find and update the scene
            updated = False
            for scene in scenes_list:
                if isinstance(scene, dict) and scene.get("scene_number") == scene_number:
            ...
            if updated:
                # Save updated JSON with correct structure
                if "visual_prompts" in prompts_data:
                    prompts_data["visual_prompts"] = scenes_list
                else:
                    prompts_data["scenes"] = scenes_list
...
class ServerMidjourneySceneGenerator:
                ...
                if result.get("code") == 200:
                    ...
                    if status == "completed":
                        ...
                        if temp_urls and len(temp_urls) > 0:
                            return {"urls": temp_urls, "source": "temporary_image_urls", "count": len(temp_urls)}
                        elif image_url:
                            return {"urls": [image_url], "source": "image_url", "count": 1}
                        else:
    ...
    def download_all_variations(self, result_data: Dict, scene_num: int) -> int:
        """Download all 4 variations of a scene"""
        urls = result_data.get("urls", [])
        successful_downloads = 0

        for i, image_url in enumerate(urls, 1):
            # scene_01_1.png, scene_01_2.png, scene_01_3.png, scene_01_4.png
            save_path = self.scenes_dir / f"scene_{scene_num:02d}_{i}.png"
            ...
            try:
                ...
                print(f"📥 Scene {scene_num} Variation {i}: Downloading...")
                ...
                if response.status_code == 200:
                    ...
                    print(f"✅ Scene {scene_num} Variation {i}: Downloaded ({file_size} bytes)")
                    successful_downloads += 1

                    # Her varyasyon için metadata kaydet
                    metadata = {
                    ...
                    json_path = self.scenes_dir / f"scene_{scene_num:02d}_{i}.json"
                    with open(json_path, 'w', encoding='utf-8') as f:
                        json.dump(metadata, f, indent=2, ensure_ascii=False)

                else:
                    print(f"❌ Scene {scene_num} Variation {i}: Download failed HTTP {response.status_code}")

            except Exception as e:
                print(f"❌ Scene {scene_num} Variation {i}: Download exception - {e}")

        print(f"📊 Scene {scene_num}: Downloaded {successful_downloads}/{len(urls)} variations")
    ...
    def get_missing_scenes(self, visual_prompts: List[Dict]) -> List[Dict]:
        ...
        for scene in regular_scenes:
            ...
            # 4 varyasyondan en az birinin eksik olup olmadığını kontrol et
            variations_exist = []
            for i in range(1, 5):  # 1, 2, 3, 4
                image_path = self.scenes_dir / f"scene_{scene_num:02d}_{i}.png"
                variations_exist.append(image_path.exists())

            # Eğer 4 varyasyondan herhangi biri eksikse, scene'i missing listesine ekle
            if not all(variations_exist):
                missing_scenes.append(scene)
                missing_count = 4 - sum(variations_exist)
                print(f"🔍 Scene {scene_num}: {missing_count}/4 variations missing")
    ...
    def generate_scenes_with_claude_correction(self, visual_prompts: List[Dict], max_retry_rounds: int = 10):
        ...
        for retry_round in range(max_retry_rounds):
            ...
            for scene_num, scene_data in completed_scenes.items():
                ...
                # 4 varyasyonu da indir
                variations_downloaded = self.download_all_variations(result_data, scene_num)

                if variations_downloaded > 0:
                    successful_downloads += variations_downloaded

                    # Ana metadata (scene özeti)
                    metadata = {
                        "scene_number": scene_num,
                        "title": scene_data["task_data"]["scene_data"]["title"],
                        "prompt": scene_data["task_data"]["prompt"],
                        "total_variations": len(result_data.get("urls", [])),
                        "downloaded_variations": variations_downloaded,
                        "generated_at": datetime.now().isoformat(),
                        "retry_round": retry_round,
                        "claude_corrections": self.claude_corrector.correction_attempts.get(scene_num, 0)
                    }

                    json_path = self.scenes_dir / f"scene_{scene_num:02d}_summary.json"
                    with open(json_path, 'w', encoding='utf-8') as f:
                        json.dump(metadata, f, indent=2, ensure_ascii=False)

            print(f"✅ Round {retry_round + 1} downloads: {successful_downloads} total variations")
