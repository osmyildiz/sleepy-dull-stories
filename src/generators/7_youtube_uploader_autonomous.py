"""
Sleepy Dull Stories - AUTONOMOUS YouTube Uploader
YouTube Video Upload with Metadata Integration + AUTONOMOUS MODE
...
class DatabaseYouTubeManager:
    ...
    def get_pipeline_status(self) -> Dict:
        ...
        cursor.execute('''
            ...
            AND (youtube_upload_status IS NULL OR youtube_upload_status = 'pending')
        ''')
        youtube_queue = cursor.fetchone()[0]

        # Count active YouTube uploads
        cursor.execute('''
            SELECT COUNT(*) FROM topics 
            WHERE youtube_upload_status = 'in_progress'
        ''')
        youtube_active = cursor.fetchone()[0]

        conn.close()

        return {
            'youtube_upload_queue': youtube_queue,
            'youtube_upload_active': youtube_active
        }
...
class ServerYouTubeUploader:
    ...
    def create_fallback_metadata(self, topic_data: Dict, project_dir: Path) -> Dict:
        """Create fallback YouTube metadata if youtube_metadata.json is missing"""
        print("🔧 Creating fallback YouTube metadata...")

        # Try to get more info from other metadata files
        title = topic_data['topic']
        description = topic_data.get('description', '')

        # Check for platform_metadata.json
        platform_file = project_dir / "platform_metadata.json"
        if platform_file.exists():
            try:
                with open(platform_file, 'r', encoding='utf-8') as f:
                    platform_data = json.load(f)
                title = platform_data.get('title_options', [title])[0]
                print(f"📋 Using title from platform_metadata: {title[:50]}...")
            except Exception as e:
                print(f"⚠️ Could not load platform_metadata: {e}")

        # Generate fallback metadata
        fallback_metadata = {
            "clickbait_titles": [
                title,
                f"The Amazing Story of {title}",
                f"You Won't Believe This Story About {title}"
            ],
            "video_description": {
                "hook": f"Discover the fascinating world of {title} in this captivating story that will transport you to another time and place.",
                "main_description": f"{description}\n\nJoin us on this incredible journey as we explore the depths of history and uncover amazing tales that shaped our world.",
                "chapters": [
                    {"time": "0:00", "title": "Introduction"},
                    {"time": "0:30", "title": "The Story Begins"},
                    {"time": "2:00", "title": "Main Story"},
                    {"time": "8:00", "title": "Conclusion"}
                ],
                "subscribe_cta": "🔔 Subscribe for more amazing historical stories and tales that will captivate your imagination! Don't forget to hit the bell icon to get notified when we upload new content.",
                "disclaimer": "This story is presented for entertainment and educational purposes. While based on historical elements, some details may be dramatized for storytelling purposes."
            },
            "tags": [
                "historical stories",
                "bedtime stories",
        ...
        # Save fallback metadata for future use
        fallback_file = project_dir / "youtube_metadata_fallback.json"
        try:
            with open(fallback_file, 'w', encoding='utf-8') as f:
                json.dump(fallback_metadata, f, indent=2, ensure_ascii=False)
            print(f"💾 Fallback metadata saved: {fallback_file}")
        except Exception as e:
            print(f"⚠️ Could not save fallback metadata: {e}")

        print("✅ Fallback YouTube metadata created")
        return fallback_metadata

    def load_youtube_metadata(self, project_dir: Path, topic_data: Dict) -> Optional[Dict]:
        """Load youtube_metadata.json from project directory with fallback"""
        ...
        if metadata_file.exists():
            try:
                ...
                print(f"⚠️ Error loading YouTube metadata: {e}")
                print("🔧 Falling back to generated metadata...")

        # Create fallback metadata if file doesn't exist or is corrupted
        return self.create_fallback_metadata(topic_data, project_dir)
    ...
    def prepare_upload_data(self, metadata: Dict, topic_data: Dict) -> Dict:
        ...
        if not description_parts:
            description_parts = [
                f"An amazing story about {topic_data['topic']}.",
                "",
        ...
        tags = metadata.get('tags', [
            "stories", "educational", "entertainment", "history"
        ])[:500]  # YouTube limit
    ...
    def upload_video(self, project_info: Dict, progress_tracker, usage_tracker):
        ...
        try:
            ...
            metadata = self.load_youtube_metadata(project_dir, project_info)
            if not metadata:
                progress_tracker.mark_stage_failed("metadata_load", "Could not create YouTube metadata")
                return None
...
def run_autonomous_mode():
    """Run autonomous mode - continuously process completed video topics for YouTube upload"""
    print("🤖 AUTONOMOUS YOUTUBE UPLOAD MODE STARTED")
    print("🔄 Will process all completed video topics continuously")
    print("⏹️ Press Ctrl+C to stop gracefully")

    # Initialize database manager for pipeline status
    db_path = Path(CONFIG.paths['DATA_DIR']) / 'production.db'
    db_manager = DatabaseYouTubeManager(str(db_path))
    ...
    def signal_handler(signum, frame):
        ...
        print("🔄 Finishing current YouTube upload and shutting down...")
        running = False
    ...
    while running:
        try:
            ...
            if status['youtube_upload_queue'] > 0:
                print(f"\n📺 Found {status['youtube_upload_queue']} completed video topics ready for YouTube upload")
                ...
                if success:
                    ...
                    print(f"\n⚠️ YouTube upload failed or no projects ready")
                ...
                print("😴 No completed video topics ready for YouTube upload. Waiting 60s...")
                for i in range(60):
                    if not running:
                        break
                    time.sleep(1)
    ...
    print(f"\n🏁 AUTONOMOUS YOUTUBE UPLOAD SHUTDOWN")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--autonomous':
        ...
        try:
            ...
            print("🔧 Fallback metadata generation")
            print("🖥️ Production-ready automation")
            print("=" * 60)
