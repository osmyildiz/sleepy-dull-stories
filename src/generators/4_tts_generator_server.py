"""
Sleepy Dull Stories - SERVER-READY TTS Audio Generator
YouTube Hook + Subscribe + Voice Directions + Enceladus Voice
...
class ServerConfig:
    ...
    def setup_audio_config(self):
        ...
        print(f"💰 Budget controls: ${self.audio_config['budget_controls']['max_cost_per_story_usd']}/story, ${self.audio_config['budget_controls']['max_cost_per_session_usd']}/session")
...
class UsageTracker:
    ...
    def add_request(self, char_count, chunk_name, duration_seconds=0):
        ...
        total_cost = self.get_total_cost()
        print(f"      💰 Cost: ${request_cost:.6f} | Story: ${self.story_cost:.4f} | Session: ${total_cost:.4f} | Chars: {self.total_characters:,}")
...
class ServerTTSGenerator:
    ...
    def create_scene_audio_with_robust_retry(self, scene_text, voice_direction, chunk_name, progress_tracker, tracker=None, max_retries=5):
        ...
        if scene_num:
            ...
            if len(scene_text) != original_length:
                print(f"      🛡️  Content filter applied to scene {scene_num} ({original_length} → {len(scene_text)} chars)")
    ...
    def process_scene_based_audio_generation_with_retry(self, stories_data, hook_and_subscribe, voice_directions,
                                                        available_scenes, output_file, story_id, quality="youtube", max_retry_rounds=5):
        ...
        # Quality settings
        settings = CONFIG.audio_config["quality_profiles"].get(quality, CONFIG.audio_config["quality_profiles"]["youtube"])
        ...
        for retry_round in range(max_retry_rounds):
            ...
            for chunk_name, chunk_text, chunk_direction in all_chunks:
                if not progress_tracker.is_chunk_completed(chunk_name) and not progress_tracker.is_chunk_blacklisted(chunk_name):
